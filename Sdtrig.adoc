...
|===
^| Value | Description
|0 | [#norm:sdtrig_action_breakpoint]#Raise a breakpoint exception. (Used when software wants to use the trigger module without an external debugger attached.) `xepc` must contain the virtual address of the next instruction that must be executed to preserve the program flow.#
|1 | [#norm:sdtrig_action_dmode]#Enter Debug Mode. {csr-dpc} must contain the virtual address of the next instruction that must be executed to preserve the program flow.# +
