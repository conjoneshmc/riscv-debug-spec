...
[#norm:sdtrig_min_triggers]#If Sdtrig is implemented, the Trigger Module must support at least one
trigger.# [#norm:sdtrig_unused_csr_access]#Accessing trigger CSRs that are not used by any of the
implemented triggers must result in an illegal instruction exception.#
[#norm:sdtrig_dm_mode_csr_access]#M-Mode and Debug Mode accesses to trigger CSRs that are used by any of
the implemented triggers must succeed, regardless of the current type of
the currently selected trigger.#

[[norm:sdtrig_trigger_fire]]
...
[[norm:sdtrig_dmode_trigger_nofire]]
...
|===
^| Value | Description
|0 | [#norm:sdtrig_action_breakpoint]#Raise a breakpoint exception.# (Used when software wants to use the trigger module without an external debugger attached.) [#norm:sdtrig_action_breakpoint_xepc]#`xepc` must contain the virtual address of the next instruction that must be executed to preserve the program flow.#
|1 | [#norm:sdtrig_action_dmode]#Enter Debug Mode.# [#norm:sdtrig_action_dmode_dpc]#{csr-dpc} must contain the virtual address of the next instruction that must be executed to preserve the program flow.# +
[#norm:sdtrig_action_dmode_check]#This action is only legal when the trigger's {mcontrol-dmode} is 1.  Since
{csr-tdata1} is WARL, hardware must prevent it from containing {tdata1-dmode}=0
and action=1.# +
[#norm:sdtrig_action_dmode_needs_sdext]#This action can only be supported if `Sdext` is implemented on the hart.#
| 2 | [#norm:sdtrig_action_trace_on]#Trace on, described in the trace specification.# 
| 3 | [#norm:sdtrig_action_trace_off]#Trace off, described in the trace specification.#
| 4 | [#norm:sdtrig_action_trace_notify]#Trace notify, described in the trace specification.#
| 5 | [#norm:sdtrig_action_trace_reserved]#Reserved for use by the trace specification.#
| 8 - 9 | [#norm:sdtrig_action_external]#Send a signal to TM external trigger output 0 or 1 (respectively).#
| other | [#norm:sdtrig_action_reserved]#Reserved for future use.#
|===
...
[#norm:sdtrig_trigger_precision]#This table only applies if triggers are precise. Otherwise triggers will
fire some indeterminate time after the event, and the priority is
irrelevant.# [#norm:sdtrig_trigger_chained_priority]#When triggers are chained, the priority is the lowest
...
[#norm:sdtrig_trigger_same_priority_hit]#When multiple triggers in the same priority fire at once, {mcontrol-hit} (if
implemented) is set for all of them.# [#norm:sdtrig_trigger_same_priority_tval]#If more than one of these triggers
has {mcontrol-action}=0 then `tval` is updated in accordance with one of them, but which one is UNSPECIFIED.# [#norm:sdtrig_trigger_same_priority_dmode]#If one of these triggers has the "enter Debug Mode" action (1)
...
[[norm:sdtrig_mcontrol_no_reentry]]
...
. [#norm:sdtrig_mcontrol_atomic_lr]#`lr` instructions are loads.#
. [#norm:sdtrig_mcontrol_atomic_sc]#Successful `sc` instructions are stores.#
. It is UNSPECIFIED whether failing `sc` instructions are stores or not.
. [#norm:sdtrig_mcontrol_atomic_load]#Each AMO instruction is a load for the read portion of the operation.
...
. [#norm:sdtrig_mcontrol_atomic_store]#Each AMO instruction is a store for the write portion of the operation.
...
[[norm:sdtrig_mcontrol_combined_access]]
...
[[norm:sdtrig_mcontrol_cache_access]]
...
[[norm:sdtrig_mcontrol_cache_address_match]]
...
[[norm:sdtrig_mcontrol_cache_hint]]
...
[[norm:sdtrig_tdata2_address_invalid]]
...
[#norm:sdtrig_trigger_no_partial_exec]#Alternatively, it may state that partial execution is not allowed,
...
unless it already knows what is supported. [#norm:sdtrig_tdata_writes]#Writes to one `tdata`
...
with the new value of {csr-tdata1}. To help with this situation, [#norm:sdtrig_tdata1_trigger_disable]#it is guaranteed
...
[[norm:sdtrig_trigger_unimplemented]]
